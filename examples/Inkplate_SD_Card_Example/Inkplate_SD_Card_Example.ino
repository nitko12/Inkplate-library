...
void setup() {
  ...
  if (display.sdCardInit()) {
    ...
    if(!display.drawBitmapFromSD("image1.bmp", 0, 0)) {
      //If is something failed (wrong filename or wrong bitmap format [REMEMBER! You can only use Windows Bitmap file with color depth of 1 or 24 bits with no compression!] write error on epaper.
  ...
  //And the last thing, try to load some text from SD card. Because, we want to write vrey tinny text, it's better to use 1 bit mode, so we switch to it.
