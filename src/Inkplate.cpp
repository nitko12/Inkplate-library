/***************************************************
This library is used for controling ED097OC1 epaper panel on e-radionica's Inkplate10 dev board (we are still working on
it!). If you don't know what Inkplate is, check it out here: https://inkplate.io/
...
void Inkplate::partialUpdate()
{
    ...
    /*
      ...
      for (int i = 0; i < 600; i++) {
        ...
        _send = ((data & B00000011) << 4) | (((data & B00001100) >> 2) << 18) | (((data & B00010000) >> 4) << 23) |
    (((data & B11100000) >> 5) << 25); hscan_start(_send); data = discharge[*(D_memory_new + _pos) & 0x0F]; _send =
    ((data & B00000011) << 4) | (((data & B00001100) >> 2) << 18) | (((data & B00010000) >> 4) << 23) | (((data &
    B11100000) >> 5) << 25); GPIO.out_w1ts = (_send) | CL; GPIO.out_w1tc = DATA | CL; _pos--; for (int j = 0; j < 99;
    j++) { data = discharge[(*(D_memory_new + _pos) >> 4)]; _send = ((data & B00000011) << 4) | (((data & B00001100) >>
    2) << 18) | (((data & B00010000) >> 4) << 23) | (((data & B11100000) >> 5) << 25); GPIO.out_w1ts = (_send) | CL;
            GPIO.out_w1tc = DATA | CL;
          data = discharge[*(D_memory_new + _pos) & 0x0F];
          _send = ((data & B00000011) << 4) | (((data & B00001100) >> 2) << 18) | (((data & B00010000) >> 4) << 23) |
    (((data & B11100000) >> 5) << 25); GPIO.out_w1ts = (_send) | CL; GPIO.out_w1tc = DATA | CL; _pos--;
        }
        GPIO.out_w1ts = (_send) | CL;
        GPIO.out_w1tc = DATA | CL;
        vscan_end();
    ...
    for (int k = 0; k < rep; k++)
    {
        ...
        for (int i = 0; i < E_INK_HEIGHT; i++)
        {
            ...
            for (int j = 0; j < (E_INK_WIDTH / 8) - 1; j++)
            {
                // dram = * (D_memory_new + _pos);
                // data = LUTB[(dram >> 4) & 0x0F];
                //_send = ((data & B00000011) << 4) | (((data & B00001100) >> 2) << 18) | (((data & B00010000) >> 4) <<
...
void Inkplate::cleanFast2(uint8_t c, uint8_t n, uint16_t d)
{
    ...
    uint32_t _send = ((data & B00000011) << 4) | (((data & B00001100) >> 2) << 18) | (((data & B00010000) >> 4) << 23) |
    (((data & B11100000) >> 5) << 25); begin_frame(); for (int k = 0; k < n; k++) {
    ...
    for (int k = 0; k < 1; k++) {
      ...
      for (int i = 0; i < 600; i++) {
        for (int j = 0; j < 100; j++) {
          data = discharge[(*(D_memory_new + _pos) >> 4)];
          _send = ((data & B00000011) << 4) | (((data & B00001100) >> 2) << 18) | (((data & B00010000) >> 4) << 23) |
    (((data & B11100000) >> 5) << 25); GPIO.out_w1tc = DATA | CL; GPIO.out_w1ts = (_send) | CL; data =
    discharge[*(D_memory_new + _pos) & 0x0F]; _send = ((data & B00000011) << 4) | (((data & B00001100) >> 2) << 18) |
    (((data & B00010000) >> 4) << 23) | (((data & B11100000) >> 5) << 25); GPIO.out_w1tc = DATA | CL; GPIO.out_w1ts =
    (_send) | CL; _pos--;
        }
        vscan_write();
      }
...
void Inkplate::display3b()
{
    ...
    for (int k = 0; k < 8; k++)
    {
        ...
        for (int i = 0; i < E_INK_HEIGHT; i++)
        {
            ...
            // waveform3Bit[pix2&0x07][k] << 2) | ( waveform3Bit[(pix2>>4)&0x07][k] << 0); pixel2 |= (
            // waveform3Bit[pix3&0x07][k] << 6) | ( waveform3Bit[(pix3>>4)&0x07][k] << 4) | ( waveform3Bit[pix4&0x07][k]
            // << 2) | ( waveform3Bit[(pix4>>4)&0x07][k] << 0);

            //_send = ((pixel & B00000011) << 4) | (((pixel & B00001100) >> 2) << 18) | (((pixel & B00010000) >> 4) <<
            // 23) | (((pixel & B11100000) >> 5) << 25); hscan_start(_send);
            hscan_start((GLUT2[k * 256 + (*(--dp))] | GLUT[k * 256 + (*(--dp))]));
            //_send = ((pixel2 & B00000011) << 4) | (((pixel2 & B00001100) >> 2) << 18) | (((pixel2 & B00010000) >> 4)
            //<< 23) | (((pixel2 & B11100000) >> 5) << 25); GPIO.out_w1ts = (_send) | CL; GPIO.out_w1tc = DATA | CL;
            ...
            for (int j = 0; j < ((E_INK_WIDTH / 8) - 1); j++)
            {
                ...
                // pixel |= ( waveform3Bit[pix1&0x07][k] << 6) | ( waveform3Bit[(pix1>>4)&0x07][k] << 4) | (
                // waveform3Bit[pix2&0x07][k] << 2) | ( waveform3Bit[(pix2>>4)&0x07][k] << 0); pixel2 |= (
                // waveform3Bit[pix3&0x07][k] << 6) | ( waveform3Bit[(pix3>>4)&0x07][k] << 4) | (
                // waveform3Bit[pix4&0x07][k] << 2) | ( waveform3Bit[(pix4>>4)&0x07][k] << 0);

                //_send = ((pixel & B00000011) << 4) | (((pixel & B00001100) >> 2) << 18) | (((pixel & B00010000) >> 4)
                //<< 23) | (((pixel & B11100000) >> 5) << 25); GPIO.out_w1ts = (_send) | CL; GPIO.out_w1tc = DATA | CL;
                GPIO.out_w1ts = (GLUT2[k * 256 + (*(--dp))] | GLUT[k * 256 + (*(--dp))]) | CL;
                GPIO.out_w1tc = DATA | CL;

                //_send = ((pixel2 & B00000011) << 4) | (((pixel2 & B00001100) >> 2) << 18) | (((pixel2 & B00010000) >>
                // 4) << 23) | (((pixel2 & B11100000) >> 5) << 25); GPIO.out_w1ts = (_send) | CL; GPIO.out_w1tc = DATA |
                // CL;
                GPIO.out_w1ts = (GLUT2[k * 256 + (*(--dp))] | GLUT[k * 256 + (*(--dp))]) | CL;
                GPIO.out_w1tc = DATA | CL;
            }
...
int Inkplate::drawGrayscaleBitmap24(SdFile *f, struct bitmapHeader bmpHeader, int x, int y)
{
    ...
    for (j = 0; j < h; j++)
    {
        for (i = 0; i < w; i++)
        {
            ...
            // So then, we are convertng it to grayscale using good old average and gamma correction (from LUT). With
            // this metod, it is still slow (full size image takes 4 seconds), but much beter than prev mentioned
            // method.
...
void Inkplate::setIntOutputInternal(uint8_t _addr, uint8_t *_r, uint8_t intPort, uint8_t mirroring, uint8_t openDrain,
                                    uint8_t polarity)
{
    intPort &= 1;
    mirroring &= 1;
